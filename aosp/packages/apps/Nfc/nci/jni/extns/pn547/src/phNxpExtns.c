...
NFCSTATUS EXTNS_EseNfcSyncInit(void)
{
    ...
    {
        ALOGD("Error in opening file Error No = %d \n",errno);
        status = NFCSTATUS_FAILED;
    }
    else
    {
        ALOGD("/dev/ese_nfc_sync driver opened for read and write the device ");
    }
#endif
    return status;
}
...
NFCSTATUS EXTNS_EseNfcSyncDeInit(void)
{
    ...
    {
        ALOGE("Ese NFC driver handle is not available\n");
    }
#endif
    return status;
}
...
static double stop_time(TimeCal *time_val)
{
    ...
    ALOGD("Total execution time = %f seconds\n",total_time);
    ...
    if (fd_ese_nfc_sync > 0)
    {
        do{
            ...
            if (ioctl(fd_ese_nfc_sync, SYNC_NFCIOCREQUEST, &sync_status) < 0)
            {
                ...
                {
                    ALOGE("%s :: SYNC_NFCIOCREQUEST Failed SYNC_IO_BUSY\n", __FUNCTION__);
                    status = NFCSTATUS_FAILED;
                }
                else
                {
                    ALOGE("%s :: SYNC_NFCIOCREQUEST Failed\n", __FUNCTION__);
                    status = NFCSTATUS_FAILED;
                    break;
                }
            }
            ...
            {
                ALOGE("%s :: SYNC_NFCIOCREQUEST sync_status %d\n", __FUNCTION__, sync_status);
                ...
                {
                    ALOGD("%s : SYNC_SPI_USED\n", __FUNCTION__);
                    ALOGD("Retrying to get the resource\n");
                    status = NFCSTATUS_FAILED;
                    /* sleep for 5 ms before retry*/
...
NFCSTATUS EXTNS_NfcReleaseResourceIoctl(double time_limit)
{
    ...
    if (fd_ese_nfc_sync > 0 )
    {
        do{
            ...
            {
                ALOGE("%s :: SYNC_NFCIOCRELEASE Failed sync_status %d\n", __FUNCTION__, sync_status);
                if (sync_status == SYNC_IO_BUSY)
                {
                    ALOGE("%s :: SYNC_NFCIOCRELEASE Failed SYNC_IO_BUSY\n", __FUNCTION__);
                    status = NFCSTATUS_FAILED;
                }
                else
                {
                    ALOGE("%s :: SYNC_NFCIOCRELEASE Failed\n", __FUNCTION__);
                    status = NFCSTATUS_FAILED;
                    break;
                }
            }
            ...
            {
                ALOGE("%s :: SYNC_NFCIOCRELEASE sync_status %d\n", __FUNCTION__, sync_status);
                if (sync_status == SYNC_IO_SUCCESS)
                {
                    ALOGD("%s : SUCCESS\n", __FUNCTION__);
                }
    ...
    {
        ALOGE("Ese NFC Syncdriver handle is not available\n");
    }
#endif
    return status;
}
