 ...
 */
typedef enum {
    NXPNCI_LINK_TYPE_UART,
    NXPNCI_LINK_TYPE_I2C,
    NXPNCI_LINK_TYPE_USB,
    NXPNCI_LINK_TYPE_INVALID,
} nfc_nxpnci_linktype;
...
typedef struct {
     ...
     * For NXPNCI, each EEPROM setting is a 4-byte entry,
     * of the format [0x00, addr_msb, addr_lsb, value].
     */
    uint8_t* eeprom_settings;

    /* The link type to which the NXPNCI is connected */
    nfc_nxpnci_linktype linktype;

    /* The device node to which the NXPNCI is connected */
     ...
     * possible for NXPNCI module. Configure address according to
     * board design.
     */
    uint8_t i2c_device_address;
} nfc_nxpnci_device_t;

static inline int nfc_nxpnci_open(const struct hw_module_t* module,
        nfc_nxpnci_device_t** dev) {
    return module->methods->open(module, NFC_NXPNCI_CONTROLLER,
        (struct hw_device_t**) dev);
}

static inline int nfc_nxpnci_close(nfc_nxpnci_device_t* dev) {
    return dev->common.close(&dev->common);
}
/*
 * End NXPNCI specific HAL
 */
