...
/* Close an opened nxpnci device instance */
static int nxpnci_close(hw_device_t *dev) {
    free(dev);
    return 0;
}
...
static int nfc_open(const hw_module_t* module, const char* name,
        hw_device_t** device) {
    if (strcmp(name, NFC_NXPNCI_CONTROLLER) == 0) {
        nfc_nxpnci_device_t *dev = calloc(1, sizeof(nfc_nxpnci_device_t));

        dev->common.tag = HARDWARE_DEVICE_TAG;
        dev->common.version = 0;
        dev->common.module = (struct hw_module_t*) module;
        dev->common.close = nxpnci_close;

        /* Example settings */
        dev->num_eeprom_settings = 0;
        dev->eeprom_settings = NULL;
        dev->linktype = NXPNCI_LINK_TYPE_INVALID;
